...
def change_regions_slot_description_coordinates(soup, base_coordinates, n_slots, slot_n):
    x_new = str(base_coordinates[0] + coordinates_deltas[n_slots][slot_n][0])
    y_new = str(base_coordinates[1] + coordinates_deltas[n_slots][slot_n][1])
    soup.find('rec', {'type': 'slot_descriptions'}).findAll('v2')[0]['x'] = x_new
    soup.find('rec', {'type': 'slot_descriptions'}).findAll('v2')[1]['x'] = x_new
    soup.find('rec', {'type': 'slot_descriptions'}).findAll('v2')[0]['y'] = y_new
    soup.find('rec', {'type': 'slot_descriptions'}).findAll('v2')[1]['y'] = y_new
    return soup
...
parser = argparse.ArgumentParser(
    ...
    description='This script upgrades minor settlements to major ones and adds building slots to settlements.',
    epilog='''
examples:

  Turning Brussels into a major 5-slot city

    python3 add_building_slot.py -u -g magistrate -b army_encampment flanders settlement_army-adminflanders
    python3 add_building_slot.py -b cannon_foundry flanders settlement_ordnanceflanders
    python3 add_building_slot.py -b conservatorium flanders settlement_cultureflanders
    python3 add_building_slot.py -b admiralty flanders settlement_navy-admin
''')
...
parser.add_argument('-c', '--clear_government_slot_bdi', action='store_true', help='Removes government slot BDI. IMPORTANT: You should only use it together with --upgrade_minor_to_major option.')
...
clear_government_slot_bdi = args.clear_government_slot_bdi
...
template_cai_building_slot_name = found_in_all_files(os.path.join(unpacked_startpos_esf_path, 'cai_building_slots'), f'>{template_building_slot_id}<', warn=False, return_first_name=True)

template_cai_building_slot_xml_path = os.path.join(unpacked_startpos_esf_path, 'cai_building_slots', template_cai_building_slot_name)
...
new_building_slot_ai_id = int('9000' + template_building_slot_ai_id)

while found_in_all_files(unpacked_startpos_esf_path, f'{new_building_slot_ai_id}', warn=verbose):
    new_building_slot_ai_id += 90000
...
settlement_soup.find('rec', {'type': 'CAI_WORLD_SETTLEMENTS'}).find('rec', {'type': 'CAI_SETTLEMENT'}).find('u4_ary').string + ' ' + str(new_building_slot_ai_id)




# CHANGING cai_building_slots minor

if upgrade_minor_to_major and clear_government_slot_bdi:
    government_cai_building_slot_soup = read_xml(template_cai_building_slot_xml_path)

    # Changing BDI
    government_cai_building_slot_soup.find('rec', {'type': 'CAI_WORLD_BUILDING_SLOTS'}).findAll('u4_ary', recursive=False)[1].string = ''
    if keep_bdi:
        government_cai_building_slot_soup.find('rec', {'type': 'CAI_WORLD_BUILDING_SLOTS'}).findAll('u4_ary', recursive=False)[3].string = \
        government_cai_building_slot_soup.find('rec', {'type': 'CAI_WORLD_BUILDING_SLOTS'}).findAll('u4_ary', recursive=False)[3].string.split(' ')[-1]
    else:
        government_cai_building_slot_soup.find('rec', {'type': 'CAI_WORLD_BUILDING_SLOTS'}).findAll('u4_ary', recursive=False)[3].string = ''
    government_cai_building_slot_soup.find('rec', {'type': 'CAI_WORLD_BUILDING_SLOTS'}).findAll('u4_ary', recursive=False)[7].string = ''
...
regions1_slot_coordinates_changes = {}
for i, region_slot in enumerate(regions1_slots):
    existing_regions1_slot_xml_path = os.path.join(unpacked_regions_esf_path, *region_slot.get('path').split('/'))
    existing_regions1_slot_soup = read_xml(existing_regions1_slot_xml_path)
    existing_regions1_slot_soup = change_regions_slot_description_coordinates(existing_regions1_slot_soup, base_coordinates, n_slots + 1, i + 1)
    regions1_slot_coordinates_changes[existing_regions1_slot_xml_path] = existing_regions1_slot_soup
for region_slot in ['road', 'walls']:
    walls_to_fortification = {'road': 'road', 'walls': 'fortification'}
    existing_regions1_slot_xml_path = os.path.join(unpacked_regions_esf_path, 'slot', f'settlement-{region}-{settlement}-settlement_{walls_to_fortification[region_slot]}.xml')
    existing_regions1_slot_soup = read_xml(existing_regions1_slot_xml_path)
    existing_regions1_slot_soup = change_regions_slot_description_coordinates(existing_regions1_slot_soup, base_coordinates, n_slots + 1, region_slot)
    regions1_slot_coordinates_changes[existing_regions1_slot_xml_path] = existing_regions1_slot_soup
...
# Changing coordinates
new_regions1_slot_soup = change_regions_slot_description_coordinates(new_regions1_slot_soup, base_coordinates, n_slots + 1, n_slots + 1)
...
if upgrade_minor_to_major:
    ...
    # Changing coordinates
    government_regions1_slot_soup = change_regions_slot_description_coordinates(government_regions1_slot_soup, base_coordinates, n_slots + 1, 1)
...
if upgrade_minor_to_major and clear_government_slot_bdi:
    write_xml(template_cai_building_slot_xml_path, government_cai_building_slot_soup)

write_xml(regions1_xml_path, regions1_soup)
write_xml(new_regions1_slot_xml_path, new_regions1_slot_soup)
for existing_regions1_slot_xml_path, existing_regions1_slot_soup in regions1_slot_coordinates_changes.items():
    write_xml(existing_regions1_slot_xml_path, existing_regions1_slot_soup)
if upgrade_minor_to_major:
